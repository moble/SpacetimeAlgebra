...
static void SpacetimeAlgebraParseWedgeBasisVector(unsigned int bvIdx, double *scale, unsigned int *bitmap) {
  unsigned b;
...
static void SpacetimeAlgebraParseSum(double coord[16], double scale, unsigned int bitmap) {
  int idx = SpacetimeAlgebra_basisElementIndexByBitmap[bitmap];
  coord[idx] += scale / (double)SpacetimeAlgebra_basisElementSignByIndex[idx];
}

static void SpacetimeAlgebraParseDataInit(struct SpacetimeAlgebraParseMultivectorData *PD) {
  PD->message[0] = 0;
  PD->value.set();
}

/** returns true when 'buf' starts with 'pattern' */
static int SpacetimeAlgebraCompareId(const char *pattern, const char *buf, int len) {
  int i = 0;
  while (pattern[i] != 0) {
...
#define T_GAMMA_0 100
#define T_GAMMA_1 101
#define T_GAMMA_2 102
#define T_GAMMA_3 103
...
*/
int SpacetimeAlgebraParseGetNextToken(const char *buf, int *startIdx, int *endIdx, int *lineIdx, int *currentLineStart) {
  int i, pointFound = 0;
  ...
  if (buf[(*startIdx)] == '^') {*endIdx = *startIdx; return T_WEDGE;} /*   ^   */
    ...
    *endIdx = (*startIdx) + 1;
    ...
    for (i = 0; i < 4; i++)
      if (SpacetimeAlgebraCompareId(SpacetimeAlgebra_basisVectorNames[i], buf + (*startIdx), (*endIdx) - (*startIdx) + 1)) return T_FIRST_BASIS_VECTOR + i; /* basis vector */
    return T_BAD_IDENTIFIER; /* bad identifier */
  }

  else return T_BAD_CHARACTER;
} /* end of SpacetimeAlgebraParseGetNextToken */

bool parseEx(struct SpacetimeAlgebraParseMultivectorData *PD, const std::string &_str, const std::string &_strSourceName)
{
  double coord[16];
  ...
  SpacetimeAlgebraParseDataInit(PD); /* should reset all */
  SpacetimeAlgebra::zero_16(coord);

  /* get the first token */
  token = SpacetimeAlgebraParseGetNextToken(str, &startIdx, &endIdx, &lineIdx, &currentLineStart);
  ...
  while (1) {
    ...
    if ((token == T_NUMBER) || ((token >= T_FIRST_BASIS_VECTOR) && (token <= T_LAST_BASIS_VECTOR))) { /* must be number or basis vector */
      ...
      if (!beDone) {
        if ((token >= T_FIRST_BASIS_VECTOR) && (token <= T_LAST_BASIS_VECTOR)) {
          SpacetimeAlgebraParseWedgeBasisVector((unsigned int)token - T_FIRST_BASIS_VECTOR, &scale, &bitmap);
          startIdx = endIdx+1;
        }
        else {
    ...
    /* add */
    SpacetimeAlgebraParseSum(coord, scale, bitmap);
...
{
  struct SpacetimeAlgebraParseMultivectorData PD;
